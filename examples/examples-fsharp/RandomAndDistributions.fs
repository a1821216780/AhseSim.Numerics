...
(**
The following custom RNGs are currently available in AHSEsim Numerics:
...
AHSEsim Numerics routines are by default initialized with a seed that combines
...
when created using AHSEsim Numerics by default, but that can be controlled either by a
