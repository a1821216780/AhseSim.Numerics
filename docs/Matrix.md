    ...
    open AHSEsim.Numerics.Distributions

Matrices and Vectors
====================

AHSEsim Numerics includes rich types for matrices and vectors.
...
to linear algebra. All places where AHSEsim Numerics can be used have a strong
...
The `Matrix<T>` and `Vector<T>` types are defined in the `AHSEsim.Numerics.LinearAlgebra` namespace.

For technical and performance reasons there are distinct implementations for each data type.
For example, for double precision numbers there is a `DenseMatrix` class in the `AHSEsim.Numerics.LinearAlgebra.Double`
...
use one of the AHSEsim.Numerics.Data packages instead.
...
If you are using AHSEsim Numerics from within F# interactive, you may want
to load the AHSEsim.Numerics.fsx script of the F# package. Besides loading
