...
AHSEsim Numerics provides a few alternatives with different characteristics
...
We need to reference AHSEsim Numerics and open the namespaces for
...
AHSEsim Numerics provides a few alternative random number generators in their own types.
...
and is often unwanted. That's why all AHSEsim Numerics RNGs are by default
...
shared between threads without manual locking. But all the RNGs provided by AHSEsim Numerics,
...
distributions in the `AHSEsim.Numerics.Distributions` namespace.
