...
for iPython with IntelliSense and embedded FSharp.Charting. Thanks to its NuGet support it can load other packages like AHSEsim Numerics on demand.
...
Since v3.3 the AHSEsim Numerics F# package includes a script `AHSEsim.Numerics.IfSharp.fsx` to do so.
