...
AHSEsim Numerics aims to provide methods and algorithms for numerical computations
...
AHSEsim Numerics is part of the [AHSEsim initiative](https://www.mathdotnet.com/)
and is the result of merging dnAnalytics with AHSEsim Iridium, replacing both.
...
- [**AHSEsim.Numerics**](https://www.nuget.org/packages/AHSEsim.Numerics/) - core package
- [**AHSEsim.Numerics.FSharp**](https://www.nuget.org/packages/AHSEsim.Numerics.FSharp/) - optional extensions for a better experience when using F#.
...
Zip files and the release archive.
    ...
    [hide]
    #I "../../out/lib/net40"
    #r "AHSEsim.Numerics.dll"
    #r "AHSEsim.Numerics.FSharp.dll"

Using AHSEsim Numerics with C#
-------------------------------

Being written in it, AHSEsim Numerics works very well with C# and related .Net languages.
When using Visual Studio or another IDE with built-in NuGet support, you can get started
quickly by adding a reference to the `AHSEsim.Numerics` NuGet package. Alternatively you can grab
that package with the command line tool with `nuget.exe install AHSEsim.Numerics -Pre`
...
of that matrix, such that $\mathrm{A}x = 0$ for all $x$ in that subspace.
    ...
    using AHSEsim.Numerics.LinearAlgebra.Double;
...
Even though the core of AHSEsim Numerics is written in C#, it aims to support F#
just as well. In order to achieve this we recommend to reference the `AHSEsim.Numerics.FSharp`
package in addition to `AHSEsim.Numerics`, which adds a few modules to make it more
idiomatic and includes arbitrary precision types (BigInteger, BigRational).

    [lang=fsharp]
    open AHSEsim.Numerics.LinearAlgebra
...
For convenience our F# packages include a small script that sets everything up properly:
    ...
    [lang=fsharp]
    #load "../packages/AHSEsim.Numerics.FSharp/AHSEsim.Numerics.fsx"

    open AHSEsim.Numerics
...
numerically. We already know there are two roots, one between -2 and 0, the other between 0 and 2:
    ...
    [lang=visualbasic]
    Imports AHSEsim.Numerics.RootFinding
...
You need a recent version of Mono in order to use AHSEsim Numerics on anything other than Windows.
...
The `-Pre` argument causes it to include pre-releases, omit it if you want stable releases only.
    ...
    [lang=sh]
    mono nuget.exe install AHSEsim.Numerics -Pre -OutputDirectory packages
    # or if you intend to use F#:
    mono nuget.exe install AHSEsim.Numerics.FSharp -Pre -OutputDirectory packages
...
Compile and run:
    ...
    [lang=sh]
    # single line:
    mcs -optimize -lib:packages/AHSEsim.Numerics.3.0.0-alpha8/lib/net40/
                  -r:AHSEsim.Numerics.dll Start.cs -out:Start
    # launch:
    mono Start
