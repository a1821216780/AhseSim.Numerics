...
AHSEsim Numerics is designed such that performance-sensitive algorithms
can be swapped with alternative implementations by the concept of providers.
There is currently only a provider for [linear algebra related routines](https://numerics.mathdotnet.com/api/AHSEsim.Numerics.Providers.LinearAlgebra.Mkl/MklLinearAlgebraProvider.htm), but there
...
like Intel MKL instead of the default purely managed provider. AHSEsim Numerics
provides such a provider as NuGet packages:

* AHSEsim.Numerics.MKL.Win
* AHSEsim.Numerics.MKL.Linux

Since these native libraries can become very big, there are also variants supporting
only a single platform, for example:

* AHSEsim.Numerics.MKL.Win-x86
* AHSEsim.Numerics.MKL.Win-x64
...
In order to make providers easier to use, since v3.6.0 AHSEsim Numerics
...
"Copy to Output Directory" for both libNumericsMKL.dll and libiomp5md.dll
...
Check the /x86 and /x64 folders in mathnet-numerics/out/MKL: you should now find the `libiomp5.dylib` and `libNumericsMKL.dll` libaries.
...
(x64 or x86, `uname -m` if you don't know) as usual:
    ...
    mono nuget.exe install AHSEsim.Numerics.MKL.Linux-x64 -Pre -OutputDirectory packages
...
and use `ldconfig` to tell where to look for the libraries, but for now we'll just copy them to `/usr/lib`:
    ...
    [lang=sh]
    sudo cp packages/AHSEsim.Numerics.MKL.Linux-x64.1.3.0/content/libiomp5.so /usr/lib/
    sudo cp packages/AHSEsim.Numerics.MKL.Linux-x64.1.3.0/content/libNumericsMKL.dll /usr/lib/
...
Compile and run:
    ...
    mcs -optimize -lib:packages/AHSEsim.Numerics.3.0.0-alpha8/lib/net40/
                  -r:AHSEsim.Numerics.dll Example.cs -out:Example
    # launch:
    mono Example
