...
using AHSEsim.Numerics.IntegralTransforms;
using AHSEsim.Numerics.Providers.MKL;
...
namespace Benchmark.Transforms
{
    ...
    public class FFT
    {
        class Config : ManualConfig
        {
            public Config()
            {
                AddJob(Job.Default.WithRuntime(ClrRuntime.Net48).WithPlatform(Platform.X64).WithJit(Jit.RyuJit));
                AddJob(Job.Default.WithRuntime(ClrRuntime.Net48).WithPlatform(Platform.X86).WithJit(Jit.LegacyJit));
