...
* Control.Describe: human readable summary of the effective AHSEsim Numerics configuration
...
* F#: packages now include a AHSEsim.Numerics.fsx script that includes FSI printers and references the assemblies.
...
* See also: [Roadmap](https://sdrv.ms/17wPFlW) and [Towards AHSEsim Numerics Version 3](https://christoph.ruegg.name/blog/towards-mathnet-numerics-v3.html).
...
### 2.6.0 - 2013-07-26
* See also: [What's New in AHSEsim Numerics 2.6](https://christoph.ruegg.name/blog/new-in-mathnet-numerics-2-6.html)
...
* Linear Algebra:
   ...
   * We're phasing out AHSEsim.Numerics.IO that used to be included in the main package for matrix file I/O for text and Matlab formats. Use the new .Data.Text and .Data.Matlab packages instead.
...
### 2.5.0 - 2013-04-14
* See also: [What's New in AHSEsim Numerics 2.5](https://christoph.ruegg.name/blog/new-in-mathnet-numerics-2-5.html)
...
* Linear Algebra: More reasonable ToString behavior for matrices and vectors. *This may break code if you relied upon ToString to export your full data to text form intended to be parsed again later. Note that the classes in the AHSEsim.Numerics.IO library are more appropriate for storing and loading data.*
* Statistics:
...
* F#: Examples (and thus the NuGet Sample package) are now F# scripts prepared for experimenting interactively in FSI, instead of normal F# files. Tries to get the assembly references right for most users, both within the AHSEsim Numerics solution and the NuGet package.
...
* Linear Algebra: Simplifications around using native linear algebra providers (see AHSEsim Numerics With Native Linear Algebra)
